...
// ------------------------------------------------------------------
// FR-12 — validasi ringan format NIK/NPWP. Ini BUKAN validasi cek-digit
// resmi (algoritma cek-digit NPWP tidak dipublikasikan) — hanya menyaring
// kesalahan format yang jelas, baik dari input manual maupun guess OCR
// (dipakai juga di lib/ocr/extractFields.ts).
// ------------------------------------------------------------------

export const NIK_REGEX = /^\d{16}$/;

// NPWP 15 digit (format lama) atau 16 digit (disamakan dengan NIK sejak
// reformasi NPWP) — dicek setelah karakter non-digit (titik/strip) dibuang.
export function isValidNpwpFormat(value: string): boolean {
  const digits = value.replace(/\D/g, "");
  return digits.length === 15 || digits.length === 16;
}

export const optionalNpwp = z
  .string()
  .optional()
  .refine((v) => !v || isValidNpwpFormat(v), {
    message: "Format NPWP tidak valid (harus 15 atau 16 digit)",
  });

// Dipakai untuk field pasangan Jenis Identitas + No. Identitas — NIK (16
// digit) hanya wajib formatnya kalau Jenis Identitas-nya KTP; Paspor/SIM/
// KITAS tidak diperiksa formatnya di sini (formatnya beragam & di luar
// scope FR-12).
function withNikCheck<Shape extends z.ZodRawShape>(
  ...
  return schema.superRefine((data, ctx) => {
    ...
    const jenis = record[jenisField];
    const no = record[noField];
    if (jenis === "KTP" && typeof no === "string" && no && !NIK_REGEX.test(no)) {
      ctx.addIssue({
        code: "custom",
        path: [noField],
        message: "NIK harus 16 digit angka untuk Jenis Identitas KTP",
      });
    }
  });
}
...
export const beneficialOwnerSchema = withNikCheck(
  z.object({
...
export const powerOfAttorneySchema = withNikCheck(
  z.object({
...
export const individualDetailSchema = withNikCheck(
  z.object({
...
export const legalArrangementDetailSchema = withNikCheck(
  z.object({
...
export const legalArrangementPartySchema = withNikCheck(
  z.object({
...
export const highRiskAdditionalInfoSchema = withNikCheck(
  z.object({
